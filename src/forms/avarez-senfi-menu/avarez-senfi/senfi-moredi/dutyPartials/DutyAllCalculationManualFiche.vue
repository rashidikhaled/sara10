...
export default {
  ...
  props: { baseNosaziCode: Object, district: Number },
  mounted () {
    this.getCodeInfo()
  },
  created () {
  ...
  methods: {
    ...
    getCodeInfo () {
      ...
      this.$services.SB.getCodeInfo(data, { config: { District: this.baseNosaziCode.District } })
        .then(async ({ data }) => {
          ...
          if (this.result.success) {
            ...
            await this.log({
              ...
              saveDesc: `بارگذاری اطلاعات در فرم ${this.title}  برای کدنوسازی ${strNosaziCode.split("-").reverse().join("-")} انجام شد.`
            })
          }
        })
        .catch(response => {
