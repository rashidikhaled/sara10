<template>
  <fit>
    ...
    <div class="q-gutter-sm q-pa-sm">
      <btn-save label="تایید" @click="updateSettings" />
  ...
  methods: {
    ...
    async addLogPose () {
      try {
        ...
        const { data } = await this.$services.SB.addLogPose(payload, {
          config: { District: this.baseNosaziCode.District }
        })
        this.addLogPoseRes = this.getResponse(data)
        if (this.addLogPoseRes.success) {
          this._GIDPoseFicheHistory =
            this.addLogPoseRes.data.GIDPoseFicheHistory
          await this.log({
