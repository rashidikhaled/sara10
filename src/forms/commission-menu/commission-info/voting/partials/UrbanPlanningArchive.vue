...
export default {
  ...
  methods: {
    ...
    async getBizCode (selecteditem = null, pCode = null, pFromSearch = false) {
      ...
      let defaultArchiveGroupTitle = (this.getApplicationParam.hasOwnProperty('DefaultArchiveGroupTitle') && this.getApplicationParam.DefaultArchiveGroupTitle) || ''
          ...
          .then(async ({ data }) => {
            ...
            if (this.getArchiveBizCodeFromNosaziCodeRes.success) {
              ...
              await this.log({
                ...
                bizCodeArchive: this.bizCode,
                saveDesc: `بارگذاری اطلاعات در آرشیو شهرسازی انجام گردید.`
              })
            } else {
              this.showArchive = false
            }
  ...
  computed: {
    ...
    config () {
      return {
        config: {
          District:
            this.baseNosaziCode.District === 0 ? 1 : this.baseNosaziCode.District
        }
      }
    }
  }
