  ...
  methods: {
    ...
    loadChart () {
      this.chartData = this.normalizeChartData(
        this.getContractReportRes.data.GetContractReportResult.Result || []
      )
      this.$nextTick(() => {
    ...
    setFilters () {
      this.model = {
        ...
        RequestType:
          this.requestType === []
            ? this.$refs.requestType.selectedItems
    ...
    async loadObj (pRequest) {
      try {
        this.showLoading()
        const { data } = await this.$services.ES.getContractReport({
          pRequest
        })
        this.getContractReportRes = this.getResponse(data)
        if (this.getContractReportRes.success) {
          let res = this.getContractReportRes.data.GetContractReportResult
          this.model = res
