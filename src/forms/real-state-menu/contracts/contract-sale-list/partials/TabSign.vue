<template>
  ...
  <fit>
        <safa-status :result="getSaleSignFingerPrintRes" />
...
export default {
  ...
  data () {
    return {
      ...
      getSaleSignFingerPrintRes: null,
      saveSignFingerPrintRes: null
    }
  },
  watch: {
    "modelSign.SecretariatNo" () {
      this.getSaleSignFingerPrint()
    },
    imgSign () {
      if (this.imgFinger) {
        // this.imgSign = null
  ...
  methods: {
    async getSaleSignFingerPrint () {
      this.showLoading()
      try {
        const { data } = await this.$services.ES.getSaleSignFingerPrint({ // این متد به دلیل کار نکردن در سرا 8 پیلود بر اساس شباهت با پیلود سرویس های دیگر گذاشته میشود امکان دارد اشتباه باشد
          PNIdRequest: this.PNIdRequest
        })
        this.getSaleSignFingerPrintRes = this.getResponse(data)
        if (this.getSaleSignFingerPrintRes.success) {
          this.resultModel = this.getSaleSignFingerPrintRes.data.GetSaleSignFingerPrintResult.RequestOwnerSignFingerPrint
    ...
    async saveObj () {
      ...
      this.showLoading()
      try {
        const { data } = await this.$services.ES.saveSignFingerPrint({ // پیلود این مورد هم کامل نیست به دلیل کار نکردن در سرا 8
