      ...
      <fit>
        <FormRow class="q-mb-sm">
          <FormControl>
            <safa-text
              label="از مبلغ"
              v-model="model.jobDuty.FromPrice"
              cdcName="FromPrice"
              label-width="65px"
              @keyup.enter="loadObj"
            />
          ...
          <FormControl>
            <safa-datepicker
              label="از تاریخ"
              v-model="model.jobDuty.FromDate"
              cdcName="FromDate"
              label-width="65px"
              @keyup.enter="loadObj"
            />
          ...
          <nosazi-code-input
            label="از کد نوسازی"
            v-model="model.jobDuty.fromBaseNosaziCode"
            cdcName="fromBaseNosaziCode"
            label-width="65px"
          />
          <nosazi-code-input
            label="تا کد نوسازی"
            v-model="model.jobDuty.toBaseNosaziCode"
...
export default {
  ...
  data () {
    return {
      ...
      formKey: "4CAFB887-0EE2-4D8A-BD42-67DF08265C87",
      name: "URequestJobDuty",
      main: true,

      model: { jobDuty: { ...defaultModel } },
      selectedItem: null,
      fiches: [],
      fichesColumns: [
        {
  ...
  methods: {
    loadObj () {
      const jobDuty = this.model.jobDuty
      if (
        !window.getConfigValue("SB") ||
        typeof window.getConfigValue("SB") !== "object" ||
        !window
          .getConfigValue("SB")
          .hasOwnProperty(jobDuty.fromBaseNosaziCode.District)
      ) {
        this.showError("منطقه مورد نظر در کد نوسازی معتبر نمی باشد!")
        return
      }
      this.showLoading()
      console.log("getDutyFichesForCommision77Payload", {
        pFromRow: 1,
        pToRow: 100,
        pDutyType: 2, // صنفی
        pFromCode: convertNosaziCodeObjectToString(jobDuty.fromBaseNosaziCode),
        pToCode: convertNosaziCodeObjectToString(jobDuty.toBaseNosaziCode),
        pFromDate: jobDuty.FromDate,
        pToDate: jobDuty.ToDate,
        pFromPayablePrice: jobDuty.FromPrice,
        pToPayablePrice: jobDuty.ToPrice,
        pEumDutyFicheStatus: "",
        pWhereCondition: "",
        pIncludeShop: true,
      ...
      this.$services.SB.getDutyFichesForCommision77(
        ...
        { config: { District: jobDuty.fromBaseNosaziCode.District } }
      )
        .then(async ({ data }) => {
    ...
    async saveProcess (fiche) {
      try {
        ...
        await this.log({
          ...
          saveDesc: `عملیات ثبت درخواست بدهی صنفی با شماره ${this.procInfo.NidWorkItem} انجام گردید.`
        })
        ...
        // this.showSuccess(
        //   `درخواست شماره ${this.procInfo.NidWorkItem} جهت بررسی بدهی صنفی با موفقیت ثبت گردید.`
        // ).onDismiss(() => {
        //   this.hideLoading()
        ...
        // this.showError(
        //   `متاسفانه عملیات ثبت درخواست بدهی صنفی در کمیسیون ماده 77 برای کد نوسازی ${fiche.NosaziCode} متوقف شد.`
        // )
        this.hideLoading()
      } finally {
        this.$forceUpdate()
    ...
    async addWorkItem (fiche) {
      return new Promise(async (resolve, reject) => {
        ...
        const payload = {
          ...
          Domain: this.model.jobDuty.fromBaseNosaziCode?.District,
              ...
              ListClsRequest_Info: [
                {
                  ...
                  Request_Info: {
                    ...
                    NidRequest: newGuid,
                    NidFiche: Items.NidFiche,
                    DescRequest: this.model.jobDuty.desc,
                    NidNosaziCode: Items.NidNosaziCode,
                    NidProc: this.procInfo.NidProc,
                    NidWorkItem: this.procInfo.NidWorkItem,
                    TitleRequest: "فیش صنفی    " + Items.FicheNo,
                    IsSara10: true
                  },
                  SubFicheDetail: null
                }
    ...
    resetObj () {
      this.model.jobDuty = { ...defaultModel.jobDuty }
      // this.model = { jobDuty: { ...defaultModel } }
    }
  }
}
</script>
