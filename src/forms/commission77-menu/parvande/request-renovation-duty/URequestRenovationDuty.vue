<template>
  ...
  >
    <form-wrapper title="ثبت درخواست بدهی نوسازی">
      <template #header>
...
import FicheDetails from "./partials/FicheDetails.vue"
...
export default {
  ...
  data () {
    return {
      ...
      formKey: "58CA13B1-8851-42CA-B50C-5460EF86CC79",
      ...
      this.$services.SB.getDutyFichesForCommision77(
        {
          pFromRow: 0,
          pToRow: 100,
          pDutyType: 1, // ClsEnumEumDutyType.Renovation == 1
    ...
    async saveProcess (fiche) {
      try {
        ...
        await this.log({
          ...
          saveDesc: `عملیات ثبت درخواست بدهی نوسازی با شماره ${this.procInfo.NidWorkItem} انجام گردید.`
        })
        ...
        // this.showSuccess(
        //   `درخواست شماره ${this.procInfo.NidWorkItem} جهت بررسی بدهی نوسازی با موفقیت ثبت گردید.`
        // ).onDismiss(() => {
        //   this.hideLoading()
        ...
        // this.showError(
        //   `متاسفانه عملیات ثبت درخواست بدهی نوسازی در کمیسیون ماده 77 برای کد نوسازی ${fiche.NosaziCode} متوقف شد.`
        // )
        this.hideLoading()
      } finally {
        this.$forceUpdate()
    ...
    async addWorkItem (fiche) {
      return new Promise(async (resolve, reject) => {
        ...
        const payload = {
          ...
          Domain: this.model.renovation.fromBaseNosaziCode?.District,
              ...
              ListClsRequest_Info: [
                {
                  ...
                  Request_Info: {
                    ...
                    NidProc: this.procInfo.NidProc,
                    NidWorkItem: this.procInfo.NidWorkItem,
                    TitleRequest: "فیش نوسازی    " + Items.FicheNo,
                    IsSara10: true
                  },
                  SubFicheDetail: null
                }
