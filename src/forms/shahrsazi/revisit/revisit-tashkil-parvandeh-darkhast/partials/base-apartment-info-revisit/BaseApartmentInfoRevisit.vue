    ...
    <fit>
      <safa-tabs v-model="currentTab" :bordered="false" class="fit" height="100%">
        ...
        <tab-content name="apartmentInfo" title="مشخصات آپارتمان">
          <TabBaseApartment v-model="currentData" v-bind="passedProps" :m="mode"/>
        </tab-content>
        <tab-content name="baseOwners" title="مشخصات ثبتی و مالکین">
          <TabBaseOwner v-model="currentData" v-bind="passedProps" :m="mode"/>
        </tab-content>
        <tab-content :padding="false" name="baseUsing" title="کاربری ها" :m="mode">
          <TabBaseUsing v-model="currentData" v-bind="passedProps"/>
        </tab-content>
        <tab-content :padding="false" name="baseFront" title="پیشامدگی ها" :m="mode">
          <TabBaseFront v-model="currentData" v-bind="passedProps"/>
        </tab-content>
        <tab-content :padding="false" name="baseCommons" title="مشترکات" :m="mode">
          <TabBaseCommons
            v-model="currentData"
            :formkey="formKey"
            :title="title"
        ...
        <tab-content name="baseParking" title="سایر امکانات و پارکینگ">
      ...
      <form-actions
        ...
        editSPId="73300bda-c98e-4541-b329-5c66c32593d6"
        @cancel="load"
      ...
      >
        <template #after>
          <safa-securityPanel
            id="2d1d3713-87eb-483a-800b-e20d84f15246"
            caption="کپی از مجاز پایانکار"
          >
            <btn-default label="کپی از مجاز پایانکار" @click="handleCopyFromMojazPayankar"/>
          </safa-securityPanel>
        </template>
      </form-actions>
    </fit>
  ...
  methods: {
      ...
      const handler = async () => {
        try {
          ...
          await this.log({
            ...
            saveReason: 'کپی اطلاعات از مجاز پایانکار'
          })
          // end
        } catch (e) {
          console.error(e)
    ...
    async save () {
      ...
      try {
        ...
        this.showSending()
        const { data } = await this.$services.SC.saveRevisitApartmentParvandeh(
          {
