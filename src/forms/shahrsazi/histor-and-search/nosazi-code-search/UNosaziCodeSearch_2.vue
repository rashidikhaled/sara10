<template>
  <fit>
    ...
    <template>
      ...
      <fit>
        <safa-grid
          ...
          fit
          ignoreCdc
          title="نتایج جستجوی ملک"
          cdcName="SearchList"
          @grid:ready="onGridReady"
          @selection-changed="handleGridSelectedChanged"
    ...
    <safa-popup v-model="parvandehDialogState" width="1300px" height="700px">
      <u-parvandeh-info
        :preferNosaziCode="baseLibResults.MainObj"
...
import { compareNosaziCodeObjects, convertStringToNosaziCodeObject } from 'src/utils/nosaziCodeOperation'
...
export default {
  ...
  methods: {
    ...
    rowClickHandler: debounce(function (params) {
      this.selectedFromMelkSearch = params.data.NosaziCode
      this.showCodeOnMap(params.data.NosaziCode)
    }),
        ...
        getRows: async (params) => {
          ...
          try {
            ...
            if (this.result.success) {
              ...
              if (this.result.data) {
                const list = this.result.data.data.SearchList.sort((x, y) =>
                  compareNosaziCodeObjects(x, y)
                ).map((x) => ({ ...x, Selected: true }))

                this.showAllRowsOnMap()
                this.SearchList = list
                params.success({
                  rowData: list,
                  rowCount: this.getTotalCount(list)
                })
              }
            } else {
    ...
    showAllRowsOnMap () {
      const nosaziCodes = this.SearchList.filter((x) => x.selected).map(
        (x) => x.NosaziCode
      )
    ...
    async handleReset () {
      ...
      this.Base_NosaziCode1 = { Block: '', House: '' }
      ...
      //  let y = Object.values(model).forEach(x => {
      //     this[x] = model[x];
      //   });
      //   console.log('reset ................' , y)
    },
