  ...
  >
    <fit>
      ...
      >
        ...
        <tab-content name="croquis" title="کروکی" :padding="false">
          <TabCroque
            v-model="currentData"
        ...
        <tab-content name="plans" title="طرح تفصیلی و توضیحات">
          <TabPlans v-model="currentData" v-bind="passedProps" />
        </tab-content>
        <tab-content name="baroKaf" title="بروکف">
          <TabBarokaf
            v-model="currentData"
            v-bind="passedProps"
            :formKey="formKey"
      ...
      <form-actions
        ...
        @save="save"
        @cancel="handleCancel"
        editSPId="f28742b0-7f70-4076-9d13-519dbdb25cd3"
        class="q-py-sm q-pl-sm"
      >
        <template #after>
          <btn-default
            label="کپی اطلاعات"
            @click="copyParvandehInfo = true"
            spId="cef865d4-2602-4d76-8886-4afc99df353e"
          ...
          <btn-default
            ...
            spId="d8d09da4-07b7-49af-811d-7867fe3e129b"
            spCaption="کپی به مجاز پایانکار"
          />
          <btn-default label="گزارش" @click="print" />
          <btn-default
            :label="
              activeTabGlobal.currentTab === 'croquis' && !isEditable
                ? 'مشاهده کروکی'
                : 'رسم کروکی'
            "
            v-if="activeTabGlobal.currentTab === 'croquis'"
            @load="load"
    ...
    <safa-popup
      v-model="showCroquie"
      :title="
        activeTabGlobal.currentTab === 'croquis' && !isEditable ? 'مشاهده کروکی' : 'رسم کروکی'
      "
      width="980px"
      height="730px"
    >
...
import TabOwnerHistory from './partials/TabOwnerHistory.vue'
...
export default {
  ...
  data () {
    return {
      title: "شهرسازی- تشکیل پرونده عرصه",
      formKey: "ec08eea8-519c-4aae-9d9a-0a2a4c6f893c",
      result: null,
      name: "BaseHouseInfoParvandeh",
      resultSave: null,
      currentTab: "address",
  ...
  methods: {
    ...
    // normalizeShBaroKaf () {
    //   return this.currentData.Sh_BaroKaf.map(m => {
    //     return {
    //       ...m
    //     }
    //   })
    ...
    async save () {
      ...
      try {
        ...
        const ShBaroKaf = this.currentData.Sh_BaroKaf
        ShBaroKaf.BarKafComments = ShBaroKaf.BarKafComments || null
        ...
        this.currentData.Base_Commission77History =
          this.normalizeBaseCommission77History()
        ...
        const { data } = await this.$services.SC.getParvandehHouse(
          {
            ...
            PLoadFun:
              "Base_NosaziCode,Base_Owner,Base_OwnerHistory,Base_AddressInfo,Base_Edge,Base_Wall,Base_RegisterPlack,Base_GreenSpace,Base_Neighbor,Base_OtherEquipment,Base_Scuttle,Base_SubScription,Base_HouseInfo,Base_CommonEstate,Base_PreCodeInfo,Base_ShahrsaziArchive,Base_Attachment,Base_PlanMosavab,Base_PlanMojaz,Base_Door,MapImage,Base_CommissionHistory,Base_Bezel,Base_File,Base_ResourceInfo,Base_RenovationPaymentHistory,Base_AddressPostCode,Base_BlockInfo,Base_Commission77History,Base_MultiBuilding,Base_HouseInfo_Config,Base_HouseInfo_Revisit,Sh_BaroKaf"
            ...
            // 'Base_OwnerHistory,Base_HouseInfo_Config,Base_HouseInfo_Revisit,Sh_BaroKaf
          },
          this.config
        )
        this.result = this.getResponse(data)
    ...
    async print () {
      const reportPath = "/Sara8Reports/RptHouseInfo"
      const queryParams = {
