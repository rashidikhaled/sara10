...
export default {
  ...
  data () {
    return {
      ...
      title: 'شهرسازی- مجاز پروانه دستگاه',
      main: true,
      result: null,
      resultSave: null,
  ...
  methods: {
      ...
      const handler = async () => {
        try {
          ...
          await this.log({
            ...
            saveDesc: 'کپی اطلاعات مجاز پروانه'
          })
          // end
        } catch (e) {
          console.error(e)
    ...
    async save () {
      ...
      try {
        ...
        commonEstate.CI_HouseUser = commonEstate.CI_HouseUser || 0

        const { data } = await this.$services.SC.saveMojazParvanehHouseSharing(
          {
    ...
    async load () {
      ...
      try {
        ...
        const { data } = await this.$services.SC.getMojazParvanehHouseSharing(
          {
            PNidNosaziCode: this.value.NidNosaziCode,
            PLoadFun:
              'Base_NosaziCode,Base_Using,Base_Front,Base_Installation,Base_SupplyParking,Base_CommonEstate,Base_BuildingInfo,Base_Gap,Base_Door,Base_Wall,Base_Bezel,Base_OtherEquipment,Base_ControllerInfo,Base_Attachment'
          },
          this.config
        )
        this.result = this.getResponse(data)
