...
export default {
  name: 'USTBaseHouseInfoParvandeh',
  ...
  methods: {
    ...
    handleCopyAction () {
      ...
      this.showConfirm('آیااز کپی اطلاعات اطمینان دارید؟')
        .onOk(() => {
          this.copyInfo()
        })
    },
    ...
    async load () {
      ...
      try {
        ...
        const { data } = await this.$services.SC.getParvandehHouseDeleted(
          {
        ...
        if (this.result.success) {
          ...
          // this.currentData = this.result.data.map(row => {
          //   return {
          //     ...row,
          //     Base_Dig: (row.Base_Dig === null ? [] : row.Base_Dig)
          //   }
          // })
