  ...
  <safa-form :id="formKey" :caption="title" appId="ACE63A06-E835-457D-A1EA-3B477DD9E69B">
    <fit>
      ...
      <safa-tabs v-model="currentTab" class="fit" height="100%">
        ...
        <tab-content name="baseArchive" :padding="false">
          <TabArchive v-model="currentData" v-bind="passedProps" />
        </tab-content>
        <tab-content name="engineer" :padding="false">
        ...
        >
          <TabCroquie v-model="currentData" v-bind="passedProps" />
        </tab-content>
  ...
  methods: {
    async print () {
      ...
      await this.log({
        ...
        nosaziCode,
        saveDesc: `نمایش گزارش اطلاعات تشکیل پرونده دستگاه با موفقیت برای کدنوسازی ${nosaziCode
          .split("-")
          .reverse()
          .join("-")} انجام گردید`
      })
        ...
        try {
          ...
          if (this.result.success) {
            ...
            await this.log({
              ...
              bizCodeTitle: "NidBase",
              nosaziCode,
              saveDesc: `کپی به مجاز پروانه اطلاعات تشکیل پرونده دستگاه با موفقیت برای کدنوسازی ${nosaziCode
                .split("-")
                .reverse()
                .join("-")} انجام گردید`
            })
    ...
    handleCopyFromMojazParvanehToBuildingParvandeh () {
      const handler = async () => {
        try {
          ...
          this.result = this.getResponse(data)
          if (this.result.success) {
            this.showSuccess("کپی از مجاز پایانکار با موفقیت انجام شد")
            const nosaziCode = convertNosaziCodeObjectToString(
              this.baseNosaziCode
            )
            await this.log({
              ...
              bizCodeTitle: "NidBase",
              nosaziCode,
              saveDesc: `کپی از مجاز پروانه اطلاعات تشکیل پرونده دستگاه با موفقیت برای کدنوسازی ${nosaziCode
                .split("-")
                .reverse()
                .join("-")} انجام گردید`
            })
        ...
        try {
          ...
          if (this.result.success) {
            ...
            await this.log({
              ...
              bizCodeTitle: "NidBase",
              nosaziCode,
              saveDesc: `کپی به مجاز پایانکار اطلاعات تشکیل پرونده دستگاه با موفقیت برای کدنوسازی ${nosaziCode
                .split("-")
                .reverse()
                .join("-")} انجام گردید`
            })
      ...
      try {
        ...
        if (duplicateNationalCode.length > 0) {
          ...
          this.currentData.Base_Commission77History =
            this.normalizeBase_Commission77History()
          ...
          if (this.resultSave.success) {
            ...
            await this.log({
              ...
              bizCodeTitle: "NidBase",
              nosaziCode,
              saveDesc: `ذخیره اطلاعات تشکیل پرونده دستگاه با موفقیت برای کدنوسازی ${nosaziCode
                .split("-")
                .reverse()
                .join("-")} انجام گردید`
            })
    ...
    async load () {
      ...
      try {
        ...
        if (this.result.success) {
          ...
          // Sh_BaroKaf اگر به این صورت مقدار
          //  را پر کنیم ، سرویس ذخیره خطای بد ریکوئست میدهد
          // { ...this.result.data,
          //   Sh_BaroKaf: this.result.data.Sh_BaroKaf ? this.result.data.Sh_BaroKaf : {
          //     PathValue: null,
          //     BarKafComments: ''
          //   } }
          if (!this.isView) {
            ...
            await this.log({
              ...
              bizCodeTitle: "NidBase",
              nosaziCode,
              saveDesc: `بارگذاری اطلاعات تشکیل پرونده دستگاه با موفقیت برای کدنوسازی ${nosaziCode
                .split("-")
                .reverse()
                .join("-")} انجام گردید`
            })
