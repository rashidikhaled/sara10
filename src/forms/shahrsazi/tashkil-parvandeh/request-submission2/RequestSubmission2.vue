    ...
    >
      ...
      >
        ...
        <tab-content
          ...
          :padding="false"
        >
          <RequestSubmissionOld :allRequestListResults="allRequestListResults"/>
        </tab-content>
      </safa-tabs>
...
import RequestSubmissionNew from './partials/RequestSubmissionNew2.vue'
...
export default {
  ...
  data () {
    return {
      title: 'شهرسازی- ثبت درخواست قدیم',
      formKey: '43e041a3-d42b-43ed-a2b3-f5a0f45d5fa4',
  ...
  methods: {
    ...
    getShWorkFlow () {
      ...
      this.showLoading()
      this.$services.SA.getShWorkflowListInNosaziCodeObjType(WorkflowList)
        .then(async ({ data }) => {
          ...
          if (self.workflowListResult.success) {
            ...
            self.workFlowItemArray = self.workflowListResults.Sh_WorkflowList_InNosaziCodeObjType.map(
              x => ({
