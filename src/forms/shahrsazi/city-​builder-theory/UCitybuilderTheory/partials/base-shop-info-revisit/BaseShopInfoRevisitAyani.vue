  ...
  >
    <fit>
      ...
      >
        ...
        <tab-content name="baseOwner" title="آدرس و مالکین">
          <TabAddressAndOwner v-bind="passedProps" />
        </tab-content>
        <tab-content name="baseInfo" title="مشخصات">
          <TabShopInfo v-bind="passedProps" />
        </tab-content>
        <tab-content name="baseArchive" title="سوابق">
          <TabArchive v-bind="passedProps" />
        </tab-content>
        <tab-content :padding="false" name="baseUsing" title="نوع استفاده">
        ...
        <tab-content name="baseParking" title="پارکینگ و دهانه">
      ...
      <form-actions
        ...
        editSPId="1e747596-b557-4ae6-a903-7619d8efe121"
        @cancel="fetch"
...
export default {
  ...
  data () {
    return {
      formKey: "52189054-a984-47b7-bd89-393db66b9497",
      title: "شهرسازی- اطلاعات بازدید صنف",
      result: null,
      resultSave: null,
  ...
  methods: {
    ...
    async handleSaveAction () {
      try {
        ...
        this.value.Base_ShahrsaziArchiveHouse =
          this.normalizeBaseShahrsaziArchiveHouse()
        this.value.Base_Commission77History =
          this.normalizeBase_Commission77History()
        this.value.Base_CommissionHistory =
        ...
        if (this.resultSave.success) {
          ...
          await this.log({
            ...
            saveDesc: `ذخیره اطلاعات صنفی اعیانی برای شماره درخواست ${this.selectedRequest.NidWorkItem} انجام شد.`
          })
          this.fetch()
        }
      } catch (e) {
