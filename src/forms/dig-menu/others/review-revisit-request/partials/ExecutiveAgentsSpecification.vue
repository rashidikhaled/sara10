<template>
  <fit>
    <safa-grid
      ...
      :columns="ExecutiveAgentsSpecificationColumns"
...
export default {
  ...
  computed: {
    ExecutiveAgentsSpecificationColumns () {
      return [
        {
          field: "", // NIdCompany
          title: "",
  ...
  methods: {
    ...
    updateGrid () {
      this.$nextTick(() => {
        let list =
          this.value.ClsRevisit_Request?.Request_Contractor
        ...
        for (
          ...
          for (let j = 0; j < this.companies.length; j++) {
            if (
              this.value.ClsRevisit_Request.Request_Contractor[i]
                .NIdCompany === this.companies[j].NIdCompany
            ) {
              this.value.ClsRevisit_Request.Request_Contractor[i].CompanyName = `${this.companies[j].Title} --- ${this.companies[j].CompanyName}`
              this.value.ClsRevisit_Request.Request_Contractor[i].ManagerMobile = this.companies[j].ManagerMobile
              this.value.ClsRevisit_Request.Request_Contractor[i].ManagerTel = this.companies[j].ManagerTel
            }
          }
        }
      })
