  ...
  <fit>
    <FormRow class="q-mb-sm">
      <FormControl>
        <safa-combo
          ...
          v-model="
            value.ClsRenewal_RequestService.RequestService_Info.CI_DigDelayTime
          "
      ...
      <FormControl>
        <safa-combo
          label="نوع انشعاب"
          v-model="
            value.ClsRenewal_RequestService.RequestService_Info.CI_SplitType
          "
      ...
      <FormControl>
        <safa-text
          label="شماره نامه"
          v-model="value.ClsRenewal_RequestService.RequestService_Info.LetterNo"
          cdcName="LetterNo"
          label-width="100px"
          m="r"
        />
    ...
    <fit>
      <safa-grid
        ...
        v-model="value.ClsRenewal_RequestService.RequestService_Contractor"
...
export default {
  ...
  methods: {
    ...
    updateGrid () {
      this.$nextTick(() => {
        let list =
          this.value.ClsRenewal_RequestService?.RequestService_Contractor
        ...
        for (
          ...
          for (let j = 0; j < this.companies.length; j++) {
            if (
              this.value.ClsRenewal_RequestService.RequestService_Contractor[i]
                .NIdCompany === this.companies[j].NIdCompany
            ) {
              this.value.ClsRenewal_RequestService.RequestService_Contractor[i].CompanyName = `${this.companies[j].Title} --- ${this.companies[j].CompanyName}`
              this.value.ClsRenewal_RequestService.RequestService_Contractor[i].ManagerMobile = this.companies[j].ManagerMobile
              this.value.ClsRenewal_RequestService.RequestService_Contractor[i].ManagerTel = this.companies[j].ManagerTel
            }
          }
        }
      })
    ...
    selectedCompanyHandler (e) {
      if (
        this.value.ClsRenewal_RequestService.RequestService_Contractor.some(
          (s) => s.NIdCompany === e.NIdCompany
        )
      ) { return this.showError("این شرکت قبلا اضافه شده است.") }

      this.value.ClsRenewal_RequestService.RequestService_Contractor = this.value.ClsRenewal_RequestService.RequestService_Contractor.map((item) => {
