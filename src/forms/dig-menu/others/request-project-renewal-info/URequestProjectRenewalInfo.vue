<template>
  ...
  >
    <form-wrapper :title="title" :padding="false">
      ...
      <fit>
        <RequestProjectRenewalInfo v-model="model" :ficheGridHeader="ficheGridHeader" />
      </fit>
    </form-wrapper>
  </safa-form>
</template>
...
import RequestProjectRenewalInfo from "./partials/RequestProjectRenewalInfo.vue"
...
export default {
  ...
  data () {
    return {
      ...
      formKey: "67E8C2DA-49F3-42EA-8AF1-F989927FD8DD",
      main: true,
  ...
  methods: {
    async loadObj () {
      ...
      const obj = this.getLicenceExportRes.ClsExportLicense
      if (obj && obj.Request_Info !== null)
      {
        // IsApproval = this.getLicenceExportRes.data.ClsExportLicense.Request_Info.IsApproval
        // IsDisapprove = obj && obj.Request_Info.IsDisapprove ?? false
        // Nidrequest = obj && obj.Request_Info.NIdRequest
        // EumRequestStatus = obj && obj.Request_Info.EumRequestStatus ?? 0
        IsRenewal = obj?.Request_Info?.IsRenewal ?? false
        AgainRenewal = obj?.Request_Info?.AgainRenewal ?? false
      }
      else if (obj && obj.RequestService_Info !== null)
      {
        // IsApproval = obj && obj.RequestService_Info.IsApproval
        // IsDisapprove = obj && obj.RequestService_Info.IsDisapprove ?? false
        // Nidrequest = obj && obj.RequestService_Info.NIdRequestService
        // EumRequestStatus = obj && obj.RequestService_Info.EumRequestStatus ?? 0
        IsRenewal = obj?.RequestService_Info?.IsRenewal ?? false
        AgainRenewal = obj?.RequestService_Info?.AgainRenewal ?? false
      ...
      return new Promise(async (resolve, reject) => {
        const res = await this.$services.excavation.getLicenceInfo({
          pRequest: {
            ...
            NidProc: "c540fd82-6b90-4f54-8937-c2de21d9c18c", // this.selectedRequest.NidProc,
    ...
    getLicenceExport () {
      return new Promise(async (resolve, reject) => {
        const res = await this.$services.excavation.getLicenceExport({
          pRequest: {
            DoNotChangeConfirmDate: false,
            NidProc: this.selectedRequest.NidProc, // "c540fd82-6b90-4f54-8937-c2de21d9c18c",
            NidWorkItem: 0,
            Skip: 0,
            Take: 0,
            CI_Bank: 0,
