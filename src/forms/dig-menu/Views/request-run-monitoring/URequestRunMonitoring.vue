      ...
      <fit>
        <safa-splitter v-model="spliterModel" class="fit">
          <template v-slot:before>
            <q-list class="q-mr-sm" bordered separator dense padding>
              <q-intersection
                v-for="(item, index) in allRequestHeaders"
                :key="index"
                transition="fade"
                once
              >
                <q-item dense>
                  <q-item-section>
                    <q-item-label>{{ item.UserName }} {{ item.Date }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-icon
...
export default {
  ...
  data () {
    return {
      ...
      formKey: "376a35de-c026-4e17-8ae2-da4c355e7d53",
      main: true,
        ...
        if (this.getRequestRes.success) {
          ...
          if (res) {
            this.model.ActualCompletion = {
              Date: "", // res.ActualCompletion.Date ?? "",
              UserName: "", // res.ActualCompletion.UserName ?? "",
              ActualCompletionDate:
