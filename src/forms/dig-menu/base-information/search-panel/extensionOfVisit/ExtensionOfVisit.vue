    ...
    <safa-status :result="getBazdidRes" /> -->
      <safa-tabs v-model="currentTab" fit :bordered="false">
        <template v-slot:tabs>
          ...
          <tab-menu name="DescriptionLicense" label="توضیحات تمدید مجوز" />
        ...
        <tab-content name="DescriptionLicense">
          <DescriptionLicense v-model="model" :rowSelectData="rowSelectData" />
        </tab-content>
  ...
  methods: {
    loadObj () {
      ...
      this.$services.excavation
        .getRevisitRenewalRequest(payload)
        .then(({ data }) => {
          this.loadObjRes = this.getResponse(data)
          if (this.loadObjRes.success) {
            this.model = this.loadObjRes.data.GetRevisitRenewal_RequestResult
