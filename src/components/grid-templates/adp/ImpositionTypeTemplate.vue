  ...
  <td :style="'min-width: ' + tdWidth">
    <div v-show="canEdit" class="row no-wrap items-center justify-space-between q-col-gutter-x-sm">
      <div :class="`${canEdit?'col':'col-auto'}`" style="min-width: 90px;">
        <safa-select @update:item="updateSelectedGroup" :ci-name="ciName" :domainName="domainName" @input="updateGroup" :value="groupValue" :m="canEdit?'e':'r'"></safa-select>
      </div>
...
const groupField = 'CI_Sara8Sys_ExemptionType'
const typeField = 'CI_Sara8Sys_ImpositionType'
const serviceMethod = 'GetCI_Sara8Sys_ImpositionType_FromCI_Sara8Sys_ExemptionType'
const componentName = 'ImpositionTypeTemplate'
const responseKey = 'CI_Sara8Sys_ImpositionType_FromCI_Sara8Sys_ExemptionType'
const paramName = 'pCI_Sara8Sys_ExemptionType'
  ...
  methods: {
    ...
    fetchUsingType () {
      ...
      this.$services.equipmentCI
        ...
        .then(({ data }) => {
          if (data) { this.dependItems = data[responseKey] || [] }
