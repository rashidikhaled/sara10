...
const typeField = 'CI_UsingType_Mojood'
const serviceMethod = 'GetCI_UsingType_FromCI_UsingGroup'
const componentName = 'UsingMojudTemplate'
...
export default {
  ...
  watch: {
    dataItem: {
      handler () {
        ...
        this.groupValue = this.dataItem[this.field]
      },
      deep: true
    },
    groupValue () {
  ...
  mounted () {
    // this.groupValue = this.dataItem[groupField]
    this.groupValue = this.dataItem[this.field] || 0
    this.typeValue = this.dataItem[typeField] || 0
    // this.fetchUsingType()
  }
}
</script>
