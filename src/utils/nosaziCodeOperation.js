// CONSTANTS
const ICONS = [ 'error', 'location_city', 'landscape', 'home', 'apartment', 'account_balance', 'shopping_cart' ]
...
export function bindNosaziItem (item) {
  ...
  newObj.section = SECTIONS[item.EumNosaziCodeObjType]
...
export function createTree (list = []) {
  ...
  if (!root) { root = cleanList.reduce((state, x) => x.EumNosaziCodeObjType < state.EumNosaziCodeObjType ? x : state) }
  if (root) {
    return [{
      ...root,
