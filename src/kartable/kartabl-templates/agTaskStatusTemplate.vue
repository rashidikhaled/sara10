<template>
  <div class="relative-position">
    <div class="task-status q-pr-sm">
      <div
        :class="{
          active: selectedTask === item.NidTask,
          'is-simple': isSimpleTask(item),
        }"
        :key="index"
      ...
      >
        ...
        >
          <div class="row q-gutter-x-sm no-wrap items-center full-height">
              ...
              <div class="row items-center no-wrap q-col-gutter-x-xs">
                <div class="col-auto q-gutter-x-xs">
                  <q-btn
                    ...
                    rounded
                    dense
                    @click.prevent.stop="showTaskUsersGroup(item)"
                  />
                  <user-avatar
                    :src="(item.AssingTo || '') | avatar"
                    :title="item.AssingToUserName || ''"
                    size="26px"
                    class="shadow-1"
                  />
            ...
            <div
              class="col-auto flex items-center no-wrap"
            ...
            >
              ...
              <span
                ...
                :style="{ color: getTaskStatusColor(item) }"
                >{{ getTaskStatusLabel(item) }}</span
              >
            </div>
            <div
              class="col-auto text-center flex items-center justify-center"
            ...
            <div class="col-auto" style="min-width: 24px">
              <q-btn
                ...
                color="primary"
                dense
                :flat="selectedTask !== item.NidTask"
                :outline="selectedTask === item.NidTask"
                icon="more_horiz"
                round
                size="sm"
    ...
    <div class="collapse-toggle" style="right: 4px" v-if="list.length > 1">
      <q-btn
        @click="params.data.showAll = true"
        color="grey-5"
        dense
        icon="add"
        size="6px"
        v-if="!params.data.showAll"
      />
      <q-btn
        @click="params.data.showAll = false"
        color="grey-5"
        dense
        icon="remove"
        size="6px"
...
export default {
  ...
  computed: {
    ...
    /* list () {
                        try {
                          return JSON.parse(this.params.data[this.field])
                        } catch {
                          return []
                        }
                      } */
  ...
  methods: {
    ...
    getTaskBgColor (task) {
      ...
      return this.isCitizen(task)
        ? "linear-gradient(to right, #eefff6, #eafef3, #e5fcf0, #e1fbed, #ddf9ea)"
        : "linear-gradient(to right, #edf8fb, #eaf8fc, #e7f8fd, #e3f9fe, #e0f9ff)"
    },
    getTaskColor (task) {
    ...
    bindList () {
      ...
      try {
        this.list = this.params.data["Task"].map((item, index) => {
          return {
            ...item,
    ...
    getTimeColor (task) {
      ...
      const start = new PersianDate(
        task.TaskStartDate.split("/").map((x) => parseInt(x))
      ).toCalendar("persian")
      ...
      if (parseInt(task.EumTaskStatus) === 1) {
        try {
          const complete = new PersianDate(
            task.CompleteDate.split("/").map((x) => parseInt(x))
          ).toCalendar("persian")
    ...
    getTaskFormatDate (date) {
      return new PersianDate(date.split("/").map((x) => parseInt(x)))
        .toCalendar("persian")
        .toLocale("fa")
        .format("DD MMMM")
    },
    selectTask (task) {
      ...
      setTimeout(() => {
        ...
        self.$stKartable.dispatch("setSelectedRequest", {
          ...task,
          ...self.params.data
        })
        self.$store.dispatch("engineer/selectRequest", {
          ...task,
          ...self.params.data
        })
      }, 400)
    ...
    showTaskUsersGroup (task) {
      if (this.params?.colDef?.onShowTaskUsersGroup) {
        this.params.colDef.onShowTaskUsersGroup(task)
      }
    }
  },
  mounted () {
  ...
  watch: {
    ...
    "params.data.showAll": {
      handler () {
        let taskLength = 44
        if (!this?.params?.data) return taskLength
        const taskCount = this.params.data.Task.length
        if (this.params.data.showAll) taskLength = taskCount * 42
        this.params.node.setRowHeight(taskLength)
